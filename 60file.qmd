...
Python에서 파일을 읽을 때는 **`open()` 함수**로 파일을 연 후, 다양한 방법으로 파일을 읽을 수 있다.  
파일을 읽은 후에는 반드시 **`close()`**를 호출하거나 **`with open()`**을 사용해 자동으로 파일을 닫아야 한다.  
...
파일을 읽기 전에 파일이 존재하는지 확인하려면 **`os.path.exists()`**를 사용할 수 있다.  
