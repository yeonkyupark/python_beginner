...
```
- `Dog` 클래스는 하나지만, `dog1`, `dog2`처럼 여러 개의 인스턴스를 생성할 수 있다.  
- 각 인스턴스는 독립적으로 동작한다.  
...
- **생성자는 객체가 생성될 때 호출**되며, 속성을 초기화하는 역할을 한다.  
- **소멸자는 객체가 삭제되거나 프로그램이 종료될 때 호출**되며, 메모리에서 해제될 때 필요한 작업을 수행할 수 있다.  
...
- `BankAccount` 클래스는 은행 계좌의 기본적인 기능인 입금, 출금, 잔액 조회를 구현한다.
- **입금**, **출금**은 조건을 통해 **올바른 금액**만 처리하도록 한다.
- `__init__()` 메서드를 통해 **초기값**인 계좌 소유자와 잔액을 설정할 수 있다.
...
- `Product` 클래스는 **제품명**, **가격**, **재고**를 속성으로 가진다.
- 가격과 재고 변경은 **update_price()**와 **update_stock()** 메서드를 통해 이루어진다.
- **제품 정보 출력** 메서드를 통해 제품 정보를 쉽게 확인할 수 있다.
...
- `Student` 클래스는 학생의 **이름**과 **성적** 리스트를 속성으로 가진다.
- 성적을 추가할 수 있는 **add_grade()** 메서드와, 평균 성적을 계산하는 **average_grade()** 메서드를 제공한다.
