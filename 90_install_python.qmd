...
파이썬에서 제공하는 IDE(Iegegrated Development Environment, 통합 개발 환경)로 `IDLE`(Integrated Development and Learning Environment)[^idle]이 있다.  소스코드를 파일로 작성하여 실행하거나 프롬프트 `>>>` 상에서 코드를 직접 작성하여 실행할 수도 있다.
